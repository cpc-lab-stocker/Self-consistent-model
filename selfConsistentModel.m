...
for kk=1:length(stdSensory)  
    ...
    %% Calculate the incongruent error by motor noise p(Incongruent|theta)
    pIncongruentGcwTh = sum(pthhGthChcw(th' <= 0, :), 1);
    pIncongruentGccwTh = sum(pthhGthChccw(th' >= 0, :), 1);
    errorIncongruent(kk, :) = pIncongruentGcwTh .* PChGtheta_lapse(1, :) + pIncongruentGccwTh .* PChGtheta_lapse(2, :);   
...
disp(['Incongruent error (motor): ' num2str(100*mean(errorIncongruent(:)))])
